...
let PermintaanPembelian = {
    ...
    add: () => {
        window.location.href = url.base_url(PermintaanPembelian.module()) + "add?tipe=DOCT_PR";
    },
    ...
    detailWebView: (elm) => {
        ...
        window.location.href = `${url.base_url(PermintaanPembelian.module())}detail-webview?id=${id}&nik=${nik}&akses=${akses}&id_departemen=${id_departemen}`;
    },
    ...
    backWebView: (elm) => {
        let nik = $(elm).data("nik");
        let akses = $(elm).data("akses");
        window.location.href = `${url.base_url(PermintaanPembelian.module())}index-webview?nik=${nik}&akses=${akses}`;
    },
                        ...
                        "render": (data, type, row, meta) => {
                            ...
                            if (row.status != 'CREATED') {
                                ...
                                if (row.status == 'APPROVED') {
                                    label += `<span class="badge bg-label-success">Disetujui Oleh ${row.approval_history} (${row.jabatan_approval_history})</span>`;
                                }
                                if (row.status == 'REJECT') {
                                    label += `<span class="badge bg-label-danger">Ditolak Oleh ${row.approval_history} (${row.jabatan_approval_history})</span>`;
                                }
                                if (row.status == 'REVISI') {
                                    label += `<span class="badge bg-label-warning">Direvisi Oleh ${row.approval_history} (${row.jabatan_approval_history})</span>`;
                                }
                            ...
                            if (row.state == 'CREATED' || row.state == 'WAITING') {
                                label += `<span class="badge bg-label-warning">Menunggu Persetujuan dari ${row.approval} (${row.jabatan_approval}) </span>`;
                            }
    ...
    submitRevisi: (elm, e) => {
        ...
        if (validation.runWithElement(form)) {
            $.ajax({
                ...
                dataType: 'json',
                data: params,
                url: url.base_url(PermintaanPembelian.moduleApi()) + "submitRevisi",
                beforeSend: () => {
                    message.loadingProses('Proses Simpan Data...');
                },
                error: function () {
    ...
    rejectConfirm: (elm, data_id) => {
        ...
        $(".status_barang, #is_regular, #tanggal_estimasi_pemakaian").removeClass('required');
    ...
    showSpesifikasiStandar: (elm) => {
        ...
        if (params.kodeItem == undefined || params.kodeItem == '') {
            Toast.error("Informasi", "Kode Item Belum Dipilih"); return;
        }
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: params,
            url: url.base_url(PermintaanPembelian.moduleApi()) + "showSpesifikasiStandar",
    ...
    showDataKodeItem: (elm, id_text) => {
        ...
        $.ajax({
            ...
            dataType: 'html',
            data: params,
            url: url.base_url(PermintaanPembelian.moduleApi()) + "getViewKodeItem",
            beforeSend: () => {
                message.loadingProses('Proses Pengambilan Data');
            },
    ...
    getDataKodeItem: async (bindText) => {
        ...
        if (tableData.length > 0) {
            tableData.DataTable({
                ...
                "ajax": {
                    "url": url.base_url(PermintaanPembelian.moduleApi()) + "getDataKodeItem",
                    "type": "POST",
                    "data": params
                },
                "deferRender": true,
                ...
                },
                {
                    "data": "bid",
                    "render": (data, type, row, meta) => {
                        return `<button class="btn btn-outline-primary" data-kode="${row.bkode}" data-nama="${row.bnama.replaceAll('"', '&#34;').replaceAll("'", "&#39;")}" data-spec="${row.spesifikasi ? row.spesifikasi : '-'}" data-satuan="${row.bsatuan}" onclick="PermintaanPembelian.pilihDataKodeItem(this,'${bindText}')">Pilih</button>`;
                    }
                },
                {
                    "data": "bkode",
                },
                {
                    ...
                    "width": "30%",
                    "render": (data, type, row, meta) => {
                        return `NAMA ITEM : ${row.bnama}<br><small>SATUAN : ${row.bsatuan}<br>FREKUENSI : ${row.frekuensi} x Pembelian<br>SPESIFIKASI <br>${row.spesifikasi ? row.spesifikasi : '-'}</small>`
